"""Interactive function development exercise using AI agent."""

from lessons.lab_0004_ai_agent.ai_agent.llm_client import llm_client
...
def develop_custom_function():
    """Interactive function development using AI agent."""
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a Python expert helping to "
            "develop a function.",
        }
    ]
    ...
    initial_function = llm_client.generate_response(messages)
    ...
    # Notice that I am purposely causing it to forget its commentary
    # and just see the code so that
    ...
    messages.append(
        {
            ...
            "including description, parameters, "
            "return value, examples, and edge cases. Output the "
            "function in a ```python code block```.",
        }
    )
    documented_function = llm_client.generate_response(messages)
    ...
    messages.append(
        {
            ...
            "including tests for basic functionality, "
            "edge cases, error cases, and various input scenarios. "
            r"Output the code in a \`\`\`python code block\`\`\`.",
        }
    )

    test_cases = llm_client.generate_response(messages)
    # We will likely run into random problems here depending on
    # if it outputs JUST the test cases or the
    # test cases AND the code. This is the type of issue we will learn to
    # work through with agents in the course.
